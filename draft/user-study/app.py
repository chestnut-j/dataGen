...
# 在下方填充数据规范之后，在命令行输入python app.py即可运行

spec= {
  'table':'nRows(10) And nCols(5)', 
  'columns':{
    'col0': "String()",
    # 'col1': "Random('normal, loc=10, scale=5')",
    "col2": "Int() And Range(0,10)",
    "col3": "Int() And Range(0,90) And FreqIf('>60',0.2)",
    "col4": "String() And If('col3', '>=60', 'pass', 'fail')"
  }
}
...
def buildSolver(format):
  ...
  for col in columns:
    ...
    if 'if' in col:
      ...
      if con_name in others:
        if len(col['if'])>3:
          if_c = [z3.Or(z3.And(eval('m'+condition,{'m': others[con_name][i]}), d[col['name']][i]==if_value), 
                  z3.And(z3.Not(eval('m'+condition,{'m': others[con_name][i]})), d[col['name']][i]==else_value)) for i in unselected_index]
        else:
          if_c = [z3.Or(z3.And(others[con_name][i]==condition, d[col['name']][i]==if_value),others[con_name][i]!=condition) for i in unselected_index]  
      else:
        if len(col['if'])>3:
          if_c = [z3.Or(z3.And(eval('m'+condition,{'m': d[con_name][i]}), d[col['name']][i]==if_value),
                z3.And(z3.Not(eval('m'+condition,{'m': d[con_name][i]})), d[col['name']][i]==else_value)) for i in unselected_index]
          # print(if_c)
        else:
          if_c = [z3.Or(z3.And(eval('m'+condition,{'m': d[con_name][i]}), d[col['name']][i]==if_value),d[con_name][i]!=condition) for i in unselected_index] 
      solver.add(if_c)
